	...
	public static void main(String[] args) {
		...
		Recursive1DStrategy<Integer, Double> heat = new Recursive1DStrategy<Integer, Double>() {
			...
			public String getSource() {
				return "if (l - r > 0) return 0.0;"
						+ "int lb = (r<=0) ? 1 : r;"
						+ "int ub = (l>=size-1) ? size -2 : l;"
						+ "for (int a = lb; a<ub; a++) {"
						+ "for (int b=1; b<size-1; b++) {"
						+ "double val = 1/dx * ( old[(a+1) * 10 + b] - 2 * old[a * 10 + b] + old[(a-1) * 10 + b]) + "
						+ "1/dx * ( old[a * 10 + (b+1)] - 2 * old[a * 10 + b] + old[a * 10 + (b-1)]) + old[a * 10 + b];"
						+ "newM[a * 10 + b] = val;"
						+ "}"
						+ "}"
						+ "*result = 1;"
